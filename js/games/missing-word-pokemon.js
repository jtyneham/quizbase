...
function initializeGame(root, app) {
    ...
    function updateTopicLabel() {
      if (allTopicsMode) {
        topicLabel.textContent = "Topic: All";
    ...
    topicDoneButton.addEventListener("click", (event) => {
      ...
      selectedTopics = allTopicsMode
        ? new Set()
        : new Set(pendingTopics);
    ...
    window.addEventListener("keydown", (event) => {
      if (!app.isScreenActive("missingwordpokemon")) return;
      if (
        event.code === "Space" &&
...
class QuizMissingWordPokemon extends HTMLElement {
  connectedCallback() {
...
export function registerMissingWordPokemon(app) {
  appAPI = app;
  if (!customElements.get("quiz-missing-word-pokemon")) {
    customElements.define("quiz-missing-word-pokemon", QuizMissingWordPokemon);
  }
}
