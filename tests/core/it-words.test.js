...
import { IT_MISSING_WORDS, IT_TOPIC, IT_WORDS, withItHangmanPool, withItMissingWordPool } from "../../data/it-words.js";
...
test("IT is a curated standalone topic in Missing Word and Hangman", () => {
  ...
  assert.equal(IT_WORDS.length, 243);
  ...
  assert.deepEqual(
    ...
    { easy: 70, medium: 123, hard: 50 }
  );
  assert.equal(IT_MISSING_WORDS.length, 242);
  assert.deepEqual(
    IT_WORDS.filter(([answer]) => !IT_MISSING_WORDS.some(([candidate]) => candidate === answer)).map(([answer]) => answer),
  ...
  assert.equal(missingIt.length, IT_MISSING_WORDS.length);
